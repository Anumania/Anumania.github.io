...
function yyVertexFormat() {
    ...
    this.m_formatBit = 1;            // Tracks the order that elements have been added to the vertex format
    this.m_fixedFunction = false;    // was the format defined by us for use with a "fixed function" pipeline
    this.m_format = [];              // array of vertex elements
    this.m_byteSize = 0;             // size of vertex
    this.m_bitMask = 0;              // mask with ALL bits set                        
        
    this.ShaderCache = [];

    Object.defineProperties(this, {
        Format: { 
            get: function () { return this.m_format; },
            set: function (val) { this.m_format = val; }
        },
        BitMask: { 
            get: function () { return this.m_bitMask; },
            set: function (val) { this.m_bitMask = val; }
        },
        ByteSize: {
            get: function () { return this.m_byteSize; },
            set: function (val) { this.m_byteSize = val; }
        },
        FixedFunction: {
            get: function () { return this.m_fixedFunction; },
            set: function (val) { this.m_fixedFunction = val; }
        }
    });
    ...
    ///          </summary>
    // #############################################################################################
    this.CreateFormatElement = function(_type, _usage, _formatBit) {
    
        var element = { 
            offset: this.m_byteSize,    // offset from the start of the vertex                
            type: _type,           // types from the list above
    ...
    ///          </summary>
    // #############################################################################################
    this.Add = function(_type, _usage, _formatBit) {    
    
        this.m_bitMask |= _formatBit;
        
        // Create an object literal to describe this part of the vertex format
        var formatElement = this.CreateFormatElement(_type, _usage, _formatBit);        
        switch (_type) {
        ...
        this.m_format.push(formatElement);
    }
    ...
    this.AddPosition = function () {
        ...
        this.Add(yyGL.VT_FLOAT2, yyGL.VU_POSITION, this.m_formatBit);
        this.m_formatBit = this.m_formatBit << 1;
    };
    ...
    this.AddPosition3D = function () {
        ...
        this.Add(yyGL.VT_FLOAT3, yyGL.VU_POSITION, this.m_formatBit);
        this.m_formatBit = this.m_formatBit << 1;
    };
    ...
    this.AddColour = function () {
        ...
        this.Add(yyGL.VT_COLOR, yyGL.VU_COLOR, this.m_formatBit);
        this.m_formatBit = this.m_formatBit << 1;
    };
    ...
    this.AddNormal = function () {
        ...
        this.Add(yyGL.VT_FLOAT3, yyGL.VU_NORMAL, this.m_formatBit);
	    this.m_formatBit = this.m_formatBit << 1;
    };
    ...
    this.AddUV = function () {
        ...
        this.Add(yyGL.VT_FLOAT2, yyGL.VU_TEXCOORD, this.m_formatBit);
	    this.m_formatBit = this.m_formatBit << 1;
    };
    ...
    this.AddCustom = function(_type, _usage) {
	    ...
	    this.Add(_type, _usage, this.m_formatBit);
	    this.m_formatBit = this.m_formatBit << 1;
    };
    ...
    this.Equals = function (_format) {
        ...
        if (this.m_format.length !== _format.Format.length) {
            return false;
        }

        for (var i = 0; i < this.m_format.length; i++) {
        
            var formatElThis = this.m_format[i];
