...
(function(window) {
    ...
    (function(window) {
        ...
        if (window.VBArray) {
            getter = function() {
                return new Uint8Array(new window.VBArray(this.responseBody).toArray());
            };
        }
        // Okay... umm.. untyped arrays? This may break completely.
        // (Android browser 2.3 and 3 don't do typed arrays)
        else { getter = function() { return this.responseBody; }; }
   ...
   **/
    ...
    */
    Font.prototype.ondownloaded = function() {
        ...
        for (encodingRecord = 0; encodingRecord < numTables; encodingRecord++) {
        ...
        if (cmap314 !== false) {
            ...
            if (version === 4) {
                ...
                // RK :: Get all the characters that are in the font
                var s = e + 2;
                for (; i < e; i += 2, s += 2) {
                    var chE = ushort(data[i], data[i + 1]);
                    var chS = ushort(data[s], data[s + 1]);
                    if (chE != 0xffff) {
                        for (var n = chS; n <= chE; ++n) {
                            this.characters.push(n);
                        } // end for
                    } // end if
                } // end for
                
                // we need to collect the glyph info
                ...
                if (endChar !== false) {
                    ...
                    var newhex = " "
                    try{
                    newhex = btoa(chr(0) +                         // base64 padding byte
    ...
    Font.prototype.bootstrapValidation = function(printChar, timeout) {
        ...
        if (!document.defaultView.getComputedStyle) {
            this.onload();
            ErrorOnce("Error: document.defaultView.getComputedStyle is not supported by this browser.\n" +
    ...
    */
    Font.prototype.measureSegment = function(textSegment, fontSize, metrics) {
        ...
        if (new RegExp('^\s*$').test(textSegment)) { return metrics; }
