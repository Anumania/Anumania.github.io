    ...
    while(!done)
    {
        ...
        switch (g_StartUpState)
        {
            ...
            case 3:
                var timer = get_timer()
                GameMaker_Tick();
                if(instance_exists(29))
                    yyInst(0,0,29).gmltotalFrameTime = get_timer()-timer;
                break;	
        }
    }
...
count = Math.random(255);
dt = new Date();
/*loads = [
    "LOADING LOADING LOADING LOADING LOADING",
    "THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS NEVER THE END IS",
    "PREPARE FOR INTENSE KASEIGAI ACTION",
    "ALSO TRY FIREGAME",
    "ROFL",
    "SHIT MY PANTS 2014",
    "WORK IT, MUNC IT, DO IT, MAKES US",
    "PTN MIDNIGHT ALARM",
    "DANIEL BE DAMNED, THIS GAME WILL WORK",
    "APRIL 29TH LOS ANGELES",
    ...
    "YOU FLOAT IN EMPTY SPACE",
    "DO YOU MISS THE GROUND UNDER YOUR FEET",
    "ZERO-G IS ONLY FREEDOM IF YOU CAN FLY",
    "GET ON AMONG DRIP",
    "MAY YOUR DANIELS BE FOREVER DAMNED",
    "DROP IT",
    ...
    "VENI VIDI VICI",
    "GET SCRUNCTIONED",
    "SNUNCTIONS SUNCTION AND FUNCTION (FORMERLY CHUCKS)",
    "THEY KEEP THE GOOD SHIT IN THE BACK",
    "ITS NOT LIKE I CACHED YOU OR ANYTHING, IDIOT",
    "BANGARANG",
    "WHEN THE [GAME] IS [LOADING]",
    "THE BEATEN AND THE DAMNED",
    "HAW HAW",
    "25 IMPOSTORS REMAINING",
    `
    ⡯⡯⡾⠝⠘⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢊⠘⡮⣣⠪⠢⡑ \n
⠀⠀⠀⠟⠝⠈⠀⠀⠀⠡⠀⠠⢈⠠⢐⢠⢂⢔⣐⢄⡂⢔⠀⡁⢉⠸⢨⢑⠕⡌ \n
⠀⠀⡀⠁⠀⠀⠀⡀⢂⠡⠈⡔⣕⢮⣳⢯⣿⣻⣟⣯⣯⢷⣫⣆⡂⠀⠀⢐⠑⡌ \n
...
⢀⢂⢑⠀⡂⡃⠅⠊⢄⢑⠠⠑⢕⢕⢝⢮⢺⢕⢟⢮⢊⢢⢱⢄⠃⣇⣞⢞⣞⢾ \n
⢀⠢⡑⡀⢂⢊⠠⠁⡂⡐⠀⠅⡈⠪⠪⠪⠣⠫⠑⡁⢔⠕⣜⣜⢦⡰⡎⡯⡾⡽`,
    "I SPIT BILE BY THE WAYSIDE",
    "I HAVE NEVER PLAYED MAHJONG",
    "KASEIGAI PLAYERS WHEN THEY SEE A COW",
    "THE WORLD CRIES AS YOU STEP INTO THE SKY",
    "A MILLION DOLLARS TO THE MAN WHO CAN FIND MY WHITE VANS",
...
function yyRenderStandardLoadingBar(graphics, _width, _height, _total, _current, _loadingscreen) {
	...
	} else
	{
		...
		var barheight = barwidth;                              // Loading bar only 4 pixels high
		...
		if (_current != 0)
		{
			...
			// Dar gray bar
			graphics.fillStyle = changeHue("0000FF", count*3);
			graphics.fillRect(x, y, barwidth, barheight);

			graphics.fillStyle = changeHue("00FF00", count*3);
			graphics.fillRect(x, y, w, barheight);
		}
        //yyDrawCenteredText()
        if(loadId == 38){ //sus guy
            graphics.fillStyle = changeHue("FF0000", count*3)
            //_graphics.textAlign = "center";
            wrapText(graphics,selectedload,_width / 2, (_height / 2),10000,8);
            //_graphics.textAlign = "left";
        }
        else{
		    yyDrawCenteredText(graphics, _width / 2, (_height / 2), changeHue("FF0000", count*3), selectedload);
        }
	}
}
