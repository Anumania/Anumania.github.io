...
function yyVBufferBuilder(_size) {        
    ...
    this.SetBuffer = function(arrayBuffer, vertcount){
    ...
    function FindNextUsage(_usage, _type) {
    	...
    	var onefound = false;
        var vertexElements = m_vertexFormat.m_format;    
    	for (var i = 0; i < vertexElements.length; i++)	{
    		...
    		if (((_usage == -1) || (element.usage == _usage)) && (element.type == _type))
    		{
    			...
    			if((element.bit & m_currentBitMask) === 0) {
    				...
    				if (m_vertexFormat.m_bitMask === m_currentBitMask) {
    				
    				    m_currentBitMask = 0;
    				    m_vertexCount++;					
    					m_streamCurrent += m_vertexFormat.m_byteSize;
    					
    					// Check to see if we need to resize the buffer
    					if ((m_streamCurrent + m_vertexFormat.m_byteSize) >= m_arrayBuffer.byteLength) {
